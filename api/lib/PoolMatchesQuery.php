...
class PoolMatchesQuery {

    /**
     * List matches in a pool, excluding placeholders, sorted by matchNumber.
     * Joins in fighter names and match location. Ignored matches are kept
     * (controller flags them with isIgnored=true).
     */
    public static function listForPool(int $poolID): array {
        $poolID = (int)$poolID;
        $sql = "SELECT
                ...
                WHERE eM.groupID = {$poolID}
                AND eM.isPlaceholder = 0
                ORDER BY eM.matchNumber ASC, eM.matchID ASC";
        return mysqlQuery($sql, ASSOC);
    }

    /**
     * Full match-detail row, scoped to event -> tournament -> pool -> match.
     * Returns null for any missing or cross-scope link. Same columns as
     * listForPool PLUS matchTime, signOff1, signOff2.
     */
    public static function findMatchInScope(int $eventID, int $tournamentID, int $poolID, int $matchID): ?array {
        if ($eventID <= 0 || $tournamentID <= 0 || $poolID <= 0 || $matchID <= 0) {
            return null;
        }
        $sql = "SELECT
                    eM.matchID        AS matchID,
    ...
    /** Per-match option overrides for the match-detail endpoint. */
